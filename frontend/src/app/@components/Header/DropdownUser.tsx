...
import { DEFAULT_THUMB_PATH, ERROR_MSGS } from "@/constants";
...
const DropdownUser = () => {
  ...
  const logout = async () => {
    try {
      ...
      console.debug(error);
      toast(ERROR_MSGS.LOGOUT_FAILED, { type: "error" });
    }
  };
      ...
      {dropdownOpen && (
        ...
        >
          ...
          <div className="p-2.5">
            <Button
              onClick={logout}
              className="flex w-full items-center gap-2.5 rounded-[7px] p-2.5 text-sm font-medium text-dark-4 duration-300 ease-in-out hover:bg-gray-2 hover:text-dark dark:bg-dark-2 dark:text-dark-6 dark:text-white dark:hover:bg-dark-3 dark:hover:text-white lg:text-base"
            >
              <svg
                className="fill-current"
                width="18"
